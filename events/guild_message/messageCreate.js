...
module.exports = {
    ...
    async execute(client, message) {
        ...
        if(message.content.toLowerCase().replace(regexPattern, "").includes("flipreset"))
        {
            message.reply("C'est ta mère que je flip reset");
            Logger.info(`Flip reset ${message.author.username} | ${message.channel.name}`);
        }
        if(message.content.toLowerCase().replace(regexPattern, "").includes("démonte") && Math.random() < 0.30)
        {
            message.reply("C'est ta mère que je démonte");
            Logger.info(`Démonte ${message.author.username} | ${message.channel.name}`);
        }
        if( (message.content.toLowerCase().replace(regexPattern, "").includes("soulèv")
        || message.content.toLowerCase().replace(regexPattern, "").includes("soulev")) 
        && Math.random() < 0.30)
        {
            message.reply("C'est ta mère que je soulève");
            Logger.info(`Soulève ${message.author.username} | ${message.channel.name}`);
        }
        if(message.content.toLowerCase().replace(regexPattern, "").includes("neodrift")
        || message.content.toLowerCase().replace(regexPattern, "").includes("néodrift"))
        {
            message.reply("Je néo drift dans ta daronne");
            Logger.info(`Neo ${message.author.username} | ${message.channel.name}`);
        }
        if(message.content.toLowerCase().replace(regexPattern, "").includes("quoi") && Math.random() < 0.10)
        {
